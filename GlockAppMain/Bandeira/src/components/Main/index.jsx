      ...
      <S.Container>
        ...
        <S.World>
          <div className="container-World">
            {trasformResult.filter((el)=> {
              if(input == ""){
                return el 
              }else if(el.name.toLowerCase().includes(input.toLowerCase())){
                return el
              }
            }).filter((el)=> {
              if (select == ""){
                return el 
              }else if(el.region.toLowerCase().includes(select.toLowerCase())){
                return el
              }
            }).map((el, indice) => (
                ...
                <Link to="information">
                  <img
                    ...
                    onClick={({ target }) => {
                      ...
                      Api.setLanguage(
                        el.languages.map(
                          (el) => `${el.iso639_1} ${el.name} ${el.nativeName}`
                          )
                          )
                        }}
                        />
